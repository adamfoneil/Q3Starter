...
namespace Q3Starter.Controllers
{
    public static class ConfigBuilder
    {
        public static IEnumerable<MapInfo> GetMaps(string path)
        {
            ...
            var zipFiles = GetZipFiles(path, out IEnumerable<string> errorFiles);

            foreach (var zip in zipFiles)
            {
                var bspFiles = GetZipContents(zip, (e) => e.Name.ToLower().EndsWith(".bsp"), (e) => Path.GetFileNameWithoutExtension(e.Name)).ToArray();
        ...
        /// </summary>
        private static IEnumerable<string> GetZipFiles(string path, out IEnumerable<string> errorFiles)
        {
            string[] fileNames = Directory.GetFiles(path, "*.pk3", SearchOption.TopDirectoryOnly);
            errorFiles = fileNames.Where(p => !IsZipFile(p));
